...
def print_dns_info(pkt):
    ...
    """
    if pkt.dns.qry_name:
        logger.info(f"DNS request from {ip_src(pkt)} : {pkt.dns.qry_name}")
    elif pkt.dns.resp_name:
        logger.info(f"DNS Response from {ip_src(pkt)}: {pkt.dns.resp_name}")
        ...
        if not summary:
            ...
            for server in dns_servers_from_capture(filtered_dns_response):
                ...
                if has_packets(filtered_dns_server):
                    logger.info(f"DNS Server has non DNS traffic!!! {server}")
...
def http_analysis(file: str, summary: bool = True) -> None:
    ...
    if not has_packets(filtered_http):
        logger.error("No HTTP packets found")

    if has_packets(filtered_http):
        logger.info(f"HTTP Packets: {len(filtered_http)}")

        filtered_http_put_post = pyshark_filtered_capture(file, filters.HTTP_PUT_POST)
        filtered_http_put_post.load_packets()
        logger.info(f"HTTP Packets with Put or POST: {len(filtered_http_put_post)}")
        ...
        logger.info(f"HTTP Packets with content type: {len(filtered_content_type)}")

        filtered_redirects = pyshark_filtered_capture(file, filters.HTTP_REDIRECTS)
        filtered_redirects.load_packets()
        logger.info(f"HTTP Packets with redirects: {len(filtered_redirects)}")
        ...
        logger.info(
            f"HTTP Packets with NMAP User Agent: {len(filtered_nmap_user_agent)}"
        )
...
def tcp_analysis(file: str, summary: bool = True) -> None:
    ...
    if not has_packets(filtered_tcp):
        logger.error("No TCP packets found")

    if has_packets(filtered_tcp):
        logger.info(f"TCP Packets: {len(filtered_tcp)}")
        ...
        logger.info(
            f"TCP Packets with Analysis Flags: {len(filtered_tcp_analysis_flag)}"
        )
        ...
        logger.info(f"TCP SYN Packets: {len(filtered_syn)}")

        filtered_syn_ack = pyshark_filtered_capture(file, filters.TCP_SYN_ACK)
        filtered_syn_ack.load_packets()
        logger.info(f"TCP SYN ACK Packets: {len(filtered_syn_ack)}")
        ...
        logger.info(f"TCP Buffer Full Packets: {len(filtered_buff_full)}")
...
def udp_analysis(file: str, summary: bool = True) -> None:
    ...
    if not has_packets(filtered_udp):
        logger.error("No UDP packets found")

    if has_packets(filtered_udp):
        logger.info(f"UDP Packets: {len(filtered_udp)}")

        filtered_udp_home_grown = pyshark_filtered_capture(file, filters.UDP_HOME_GROWN)
        filtered_udp_home_grown.load_packets()
        logger.info(f"UDP Home Grown Packets: {len(filtered_udp_home_grown)}")
